...
export const REMOTE_METHOD_POLICY = {
  ...
  'workspace.getStatus': read(), 'actions.list': read(), 'actions.getAffectedFiles': read(), 'operations.get': read(), 'publish.status': read(), 'files.list': read(), 'files.read': read(), 'files.stat': read(), 'git.status': read(), 'git.diff': read(), 'git.log': read(), 'git.branches': read(),
...
function verifyPkce(verifier: string, challenge: string): boolean { if (!/^[A-Za-z0-9\-._~]{43,128}$/.test(verifier)) return false; return equal(createHash('sha256').update(verifier).digest('base64url'), challenge) }
function canonicalPublicBase(value: string): string | undefined { try { const url = new URL(value); if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password || url.hash || url.search || url.pathname !== '/') return undefined; if (url.protocol === 'http:' && !loopback(url.hostname)) return undefined; return url.origin } catch { return undefined } }
