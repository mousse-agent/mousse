...
export class Win32MacroExecutor implements MacroExecutor {
  async execute(
    ...
    try {
      const script = this.buildPowerShellScript(config, context)
  ...
  private buildPowerShellScript(config: MacroConfig, context: MacroRunContext): string {
    const promptEscaped = context.prompt.replace(/'/g, "''")
    const titlePattern = context.windowTitle || config.windowTitlePattern

    const stepScripts = config.steps.map((step) => this.stepToPowerShell(step, promptEscaped))
...
$hwnd = Focus-TerminalWindow -Pattern '${titlePattern}'
$offsetX = 0
$offsetY = 0
...
`.trim()
  ...
  private stepToPowerShell(step: MacroStep, promptEscaped: string): string {
    switch (step.type) {
      case 'click':
        return `
Write-Host "[macro] click (${step.x}, ${step.y})"
[Win32]::SetCursorPos($offsetX + ${step.x}, $offsetY + ${step.y})
Start-Sleep -Milliseconds 50
[Win32]::mouse_event([Win32]::MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)
[Win32]::mouse_event([Win32]::MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)
`.trim()
      case 'delay':
        return `Start-Sleep -Milliseconds ${step.ms ?? 300}`
      case 'paste':
        return `
...
`.trim()
      case 'key': {
        const key = this.mapKey(step.key || 'Enter')
        return `
Write-Host "[macro] key ${step.key}"
...
[System.Windows.Forms.SendKeys]::SendWait('${(step.text || '').replace(/'/g, "''")}')
`.trim()
      default:
        return `Write-Host "[macro] unknown step: ${step.type}"`
    }
  }
